document.addEventListener('DOMContentLoaded', () => {
    ...
    function generateNodes(numNodes) {
        ...
        for (let i = 1; i <= numNodes; i++) {
            ...
            // Generar posiciones hasta encontrar una válida
            while (!validPosition) {
                x = Math.random() * (canvas.width - 100) + 50;
                y = Math.random() * (canvas.height - 100) + 50;

                // Comprobar que el nuevo nodo no esté cerca de los existentes
    ...
    function drawGraph(nodes, edges, ctx, path = []) {
        ...
        // Ajuste del tamaño del nodo y del texto basado en el número de nodos
        const nodeSize = Math.max(40 - nodes.length, 20); // Tamaño mínimo de 20, máximo de 50
        const fontSize = Math.max(20 - Math.floor(nodes.length / 5), 10); // Tamaño mínimo de texto de 10, máximo de 20
